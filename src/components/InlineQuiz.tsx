  ...
  return (
    <div className="border-2 border-pink-400/50 bg-gradient-to-br from-pink-400/5 to-transparent">
      ...
      <div className="p-4 space-y-3">
        ...
        {showResult && (
          <div
            className={`border-l-4 pl-3 py-2 text-xs leading-relaxed ${
              isCorrect
          ...
          <div className="flex justify-between items-center flex-wrap gap-2">
            {question.hint && (
              <button
                ...
                className="text-yellow-300 text-xs uppercase tracking-widest flex items-center gap-1 hover:text-yellow-200"
              >
                <Lightbulb size={12} /> {showHint ? 'Скрыть подсказку' : 'Показать подсказку'}
              </button>
            )}
            <button onClick={retry} className="neon-btn-light text-xs">
              Попробовать снова
            </button>
          </div>
        )}

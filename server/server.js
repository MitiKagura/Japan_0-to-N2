...
const insertProgress = db.prepare(
  'INSERT OR REPLACE INTO progress (user_id, lesson_id, score, completed_at) VALUES (?, ?, ?, CURRENT_TIMESTAMP)',
);
const deleteProgress = db.prepare('DELETE FROM progress WHERE user_id = ? AND lesson_id = ?');
const getUserProgress = db.prepare(
  'SELECT lesson_id, score, completed_at FROM progress WHERE user_id = ?',
);
...
const validateRegister = (body) => {
  ...
  else if (!/^[a-zA-Z0-9_]{3,20}$/.test(body.username))
    errors.push('username: 3-20 символов, только латиница/цифры/_');
...
app.post('/api/register', async (req, res) => {
  ...
  const displayName = req.body.display_name || username;
...
app.get('/api/me', authMiddleware, (req, res) => {
  const user = db
    .prepare('SELECT id, username, email, display_name, created_at FROM users WHERE id = ?')
    .get(req.userId);
...
app.get('/api/progress', authMiddleware, (req, res) => {
  ...
  const map = {};
  for (const r of rows) map[r.lesson_id] = { score: r.score, completed_at: r.completed_at };
  res.json({ progress: map });
});

app.post('/api/progress/:lessonId', authMiddleware, (req, res) => {
  const { lessonId } = req.params;
  const score = Math.max(0, Math.min(100, Number(req.body?.score ?? 100)));
...
app.delete('/api/progress/:lessonId', authMiddleware, (req, res) => {
  const { lessonId } = req.params;
  deleteProgress.run(req.userId, lessonId);
  res.json({ ok: true });
});

app.get('/api/stats', authMiddleware, (req, res) => {
  const row = db
    .prepare('SELECT COUNT(*) as count, MAX(completed_at) as last FROM progress WHERE user_id = ?')
    .get(req.userId);
...
app.listen(PORT, '0.0.0.0', () => {
  console.log(`🌸 SAKURA://SENSEI backend running on http://0.0.0.0:${PORT}`);
